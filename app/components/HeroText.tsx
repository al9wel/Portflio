...
const HeroText = () => {
        ...
        return letters.map((char, i) => {
            ...
            return (
                <motion.span
                    ...
                    transition={{ delay, duration: 1, type: "spring", stiffness: 110, damping: 16 }}
                    whileHover={{
                        y: -8,
                        color: "#d7bd78",
    ...
    return (
        <div className={`uppercase mt-40 sm:mt-35 md:mt-25 lg:mt-15 ${michroma.className} text-center flex flex-col justify-center items-center text-color1`}>
            <div className="hidden md:flex text-[4.8rem] lg:text-[6.7rem] xl:text-[9rem]">
                {renderLetters(wordsLarge)}
            </div>
            <div className="flex flex-col md:hidden text-6xl sm:text-8xl tracking-wider sm:tracking-widest gap-0">
                <div className="flex justify-center">{renderLetters(wordsSmall1)}</div>
                <div className="flex justify-center">{renderLetters(wordsSmall2)}</div>
            </div>
        </div>
    );
};
