...
const NavBar = () => {
    ...
    return (
        <motion.header
            ...
            <nav className=" w-[95%]  lg:w-[90%] rounded-md bg-amber-4 text-color1  backdrop-blur-sm  z-50 flex justify-between items-center uppercase mt-4 py-1 md:py-2 px-2 sm:px-4 md:px-10">
                ...
                <div
                    ...
                    {navItems.map((item) => {
                        return (
                            <Link key={item.name} href={item.url} className={`hover:text-color2 text-color1/70 duration-300 hover:scale-97 hover:translate-y-[3px] border-b-3 hover:border-b-2 border-color4/50 hover:border-color2/70 rounded-full p-2`}>{item.name}</Link>
                        )
                    })}
                </div>
